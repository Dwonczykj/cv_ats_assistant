...
# extract the resume from a pdf file
resume_path = "/Users/joey/Documents/CVs/Joey Dwonczyk CV.pdf"
resume_text = extract_resume(resume_path)
# TODO: implement the extract_resume function
...
resume_paste = extract_resume(resume_path)
if not resume_paste:
...
try:
    ...
    if response.status_code == 200:
        ...
        if result.get('success'):
            ...
            {{"apply_links": ["https://example.com/job1",
            ...
            print(f"{Colors.RED}Failed to scrape the jobs page: {
                  result.get('message', 'Unknown error')}{Colors.RESET}")
            html_content = ""
    else:
        print(f"{Colors.RED}Error {response.status_code}: {
              response.text}{Colors.RESET}")
        html_content = ""
except requests.RequestException as e:
    print(f"{Colors.RED}An error occurred while scraping: {
          str(e)}{Colors.RESET}")
    html_content = ""
    ...
    print(f"{Colors.RED}An unexpected error occurred while scraping: {
          str(e)}{Colors.RESET}")
    html_content = ""
...
if html_content:
    try:
        ...
        if completion.choices and completion.choices[0].message.content:
            ...
            print(f"{Colors.GREEN}Successfully extracted {
                  len(apply_links)} apply links{Colors.RESET}")
        else:
            print(f"{Colors.RED}No apply links extracted{Colors.RESET}")
    except json.JSONDecodeError as e:
        print(f"{Colors.RED}Error decoding JSON from OpenAI response: {
              str(e)}{Colors.RESET}")
    except KeyError as e:
        print(f"{Colors.RED}Expected key not found in OpenAI response: {
              str(e)}{Colors.RESET}")
    except Exception as e:
        print(f"{Colors.RED}An unexpected error occurred during extraction: {
              str(e)}{Colors.RESET}")
else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get('success'):
                ...
                print(f"{Colors.GREEN}Data extracted for job {
                      index}{Colors.RESET}")
            else:
                print(f"")
        else:
...
if completion.choices and completion.choices[0].message.content:
    ...
    raise ValueError("No recommended jobs found")
